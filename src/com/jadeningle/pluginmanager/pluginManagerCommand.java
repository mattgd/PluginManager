...
public class PluginManagerCommand implements CommandExecutor {
	...
	private boolean hasPermission(CommandSender sender, String permission) {
		if (sender != Bukkit.getServer().getConsoleSender()) {
			...
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Response.Error.NoPermission")));
			return false;
		}
		return true;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] split) {
		if (split.length == 0) {
			String[] listArray = { "Enable", "Disable", "Load", "Unload", "Reload", "Sreload", "Show", "List",
					"ConfigReload" };
			sender.sendMessage(ChatColor.GREEN + "|------------------" + ChatColor.GRAY + "PluginManager Help"
					+ ChatColor.GREEN + "------------------|");
			for (String string : listArray)
				sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
						plugin.language.getString("Command.Description." + string)));
			return true;
		}
		...
		default:
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.NoCommand"), label, split[0])));
			return true;
		}
	}
	...
	public boolean enablePluginCommand(CommandSender sender, String label, String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Enable")));
			return true;
		}
		...
		if (p == null)
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.NoPlugin"), split[1])));
		else if (p.isEnabled())
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.AlreadyEnabled"), split[1])));
		else {
			control.enablePlugin(p);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.PluginEnabled"), split[1])));
		}
		return true;
	}
	...
	public boolean disablePluginCommand(CommandSender sender, String label, String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Disable")));
			return true;
		}
		...
		if (p == null)
			...
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.AlreadyDisabled"), split[1])));
		else {
			control.disablePlugin(p);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.PluginDisabled"), split[1])));
		}
		return true;
	}
	...
	public boolean loadPluginCommand(CommandSender sender, String label, String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Load")));
			return true;
		}
		...
		if (!toLoad.exists()) {
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.NoSuchFile"), split[1] + ".jar")));
			return true;
		}
		...
		if (desc == null) {
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Response.Error.NoDescriptionFile")));
			return true;
		}

		if (Bukkit.getPluginManager().getPlugin(desc.getName()) != null) {
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Response.Error.AlreadyLoaded")));
			return true;
		}
		...
		if ((p = control.loadPlugin(toLoad)) != null) {
			control.enablePlugin(p);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.PluginLoaded"),
							p.getDescription().getName(), p.getDescription().getVersion())));
		} else
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Error.PluginNotLoaded"), split[1])));
	...
	private boolean unloadPluginCommand(final CommandSender sender, final String label, final String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Unload")));
			return true;
		}
		...
		if (p == null)
			...
			if (control.unloadPlugin(p, true))
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						String.format(plugin.language.getString("Response.Action.PluginUnloaded"),
								p.getDescription().getName(), p.getDescription().getVersion())));
			else
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						String.format(plugin.language.getString("Response.Error.PluginNotUnloaded"), split[1])));
		}
	...
	private boolean reloadPluginCommand(final CommandSender sender, final String label, final String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Reload")));
			return true;
		}
		...
		if (p == null)
			...
			if (file == null) {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						String.format(plugin.language.getString("Response.Error.NoJar"), p.getName())));
				return true;
			}
			...
			if (!control.unloadPlugin(p, false))
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						String.format(plugin.language.getString("Response.Error.UnloadError"), split[1])));
			else if ((loaded = (JavaPlugin) control.loadPlugin(name)) == null)
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						String.format(plugin.language.getString("Response.Error.ReloadError"), split[1])));

			control.enablePlugin(loaded);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.PluginReloaded"), split[1])));
		}
	...
	private boolean sreloadPluginCommand(final CommandSender sender, final String label, final String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Sreload")));
			return true;
		}
		...
		if (p == null)
			...
			control.enablePlugin(p);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.PluginSreloaded"),
							p.getDescription().getName(), p.getDescription().getVersion())));
		}
	...
	private boolean showPluginCommand(final CommandSender sender, final String label, final String[] split) {
		...
		if (split.length < 2) {
			sender.sendMessage(ChatColor.GOLD + "/" + label + " " + ChatColor.translateAlternateColorCodes('&',
					plugin.language.getString("Command.Description.Show")));
			return true;
		}
		...
		if (p == null)
			...
			sender.sendMessage(ChatColor.AQUA + "|----"
					+ ChatColor.translateAlternateColorCodes('&',
							String.format(plugin.language.getString("Response.Show.Name"), p.getName()) + ChatColor.AQUA
									+ "----|"));
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Show.Status.Main"),
							(p.isEnabled() ? plugin.language.getString("Response.Show.Status.Enabled")
									: plugin.language.getString("Response.Show.Status.Disabled")))));
			if (p.getDescription().getDescription() != null)
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&', String.format(
						plugin.language.getString("Response.Show.Description"), p.getDescription().getDescription())));

			sender.sendMessage(ChatColor.translateAlternateColorCodes('&', String
					.format(plugin.language.getString("Response.Show.Version"), p.getDescription().getVersion())));
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Show.Main"), p.getDescription().getMain())));
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Show.File"), file.getName())));
				...
				sender.sendMessage(
						ChatColor
								.translateAlternateColorCodes('&',
										String.format(
												(p.getDescription().getAuthors().size() == 1
														? plugin.language.getString("Response.Show.Author.Single")
														: plugin.language.getString("Response.Show.Author.Multiple")),
												authors)));

			if (p.getDescription().getWebsite() != null)
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&', String
						.format(plugin.language.getString("Response.Show.Website"), p.getDescription().getWebsite())));
		}
	...
	private boolean listPluginCommand(final CommandSender sender, final String label, final String[] split) {
		...
		if (!enabledList.isEmpty())
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.List.Enabled"), enabledList)));
		if (!disabledList.isEmpty())
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
					String.format(plugin.language.getString("Response.Action.List.Disabled"), disabledList)));
	...
	private boolean reloadConfigCommand(final CommandSender sender, final String label, final String[] split) {
		...
		p.reloadConfig();
		sender.sendMessage(ChatColor.translateAlternateColorCodes('&', String
				.format(plugin.language.getString("Response.Action.ConfigReload"), p.getDescription().getName())));
		return true;
	}
}
